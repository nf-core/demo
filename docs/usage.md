...
## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/demo/usage](https://nf-co.re/demo/usage)
...
A final samplesheet file consisting of both single- and paired-end data may look something like the one below. This is for 3 samples, where `SAMPLE3_SE` has been sequenced twice.
...
```bash
nextflow run nf-core/demo -profile docker -params-file params.yaml
```
...
First, go to the [nf-core/demo releases page](https://github.com/nf-core/demo/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
